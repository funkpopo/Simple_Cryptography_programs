...
inline void MD5Trans(UINT4 state[4], unsigned char block[64])
{
...
inline void MD5Update(MD5_CTX* context, unsigned char* input, unsigned int inputLen)
{
	...
	if ((context->count[0] += ((UINT4)inputLen << 3)) < ((UINT4)inputLen << 3)) context->count[1]++; context->count[1] += ((UINT4)inputLen >> 29);
	{
		partLen = 64 - index;
	}
	if (inputLen >= partLen) 
